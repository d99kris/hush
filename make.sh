...
if [[ "${DEPS}" == "1" ]]; then
  ...
  if [ "${OS}" == "Linux" ]; then
    ...
    if [[ "${DISTRO}" == "Ubuntu" ]]; then
      sudo apt -y install git cmake build-essential || exiterr "deps failed (linux), exiting."
    else
...
# doc
if [[ "${DOC}" == "1" ]]; then
  cd build/bin && help2man -n "fake tty execution" -N -o ../../src/faketty.1 ./faketty && \
  help2man -n "suppress stdout for commands executed successfully" -N -o ../../src/hush.1 ./hush && \
  cd ../.. || exiterr "doc failed, exiting."
fi
